...
import html, json, re, secrets, threading
...
def background(fn):
    if not BUSY.acquire(False): raise ValueError('이미 직원이 일하고 있습니다. 완료 후 다시 실행하세요.')
    JOB.update(status='진행 중',detail='완료 후 새로고침하면 결과를 볼 수 있습니다.')
    def worker():
        try:
            result=fn(); JOB.update(status='완료',detail=str(result)[:1800])
        except Exception as err: JOB.update(status='확인 필요',detail=str(err)[:1800])
        finally: BUSY.release()
    threading.Thread(target=worker,daemon=True).start()

def render(project,token):
    ...
    def form(action,body,multipart=False):
        return f'<form method="post" action="/{action}"'+(' enctype="multipart/form-data"' if multipart else '')+f'><input type="hidden" name="token" value="{token}"><input type="hidden" name="project" value="{project}">{body}</form>'
    employee_cards=''
    for e in employees:
...
<p class="muted">PC가 켜져 있고 사용자 로그인 상태일 때 실행됩니다. AI 직원은 매 작업마다 새 세션으로 호출되며, 업무 기억은 사무실에 남습니다. v1 실행 범위는 문서 작성·공개 조사·주문지 정리입니다. 외부 발송·게시·결제 기능은 연결되어 있지 않습니다.</p></main></html>'''
...
def handler_class(port):
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if url.path=='/': self.send(render(parse_qs(url.query).get('office',['baek'])[0],token)); return
        ...
        def do_POST(self):
            ...
            try:
                ...
                if self.path=='/advisor': background(lambda:advisor.ask(p,fields['question']))
                    ...
                    kind='public_research' if fields['agent']=='startup_research' else 'document'
                    team.assign(fields['agent'],fields['title'][:200],fields['brief'][:8000],kind=kind)
                ...
                self.send_response(303); self.send_header('Location','/?office='+p); self.end_headers()
